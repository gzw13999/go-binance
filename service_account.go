...
func (as *apiService) NewOrderFull(or NewOrderRequest) (*ProcessedOrderFull, error) {
	params := make(map[string]string)
	params["symbol"] = or.Symbol
	params["side"] = string(or.Side)
	params["type"] = string(or.Type)
	...
	if res.StatusCode != 200 {
		fmt.Println("statuscode!=200", string(textRes))
		return nil, as.handleError(textRes)
	}
	...
	price, _ := strconv.ParseFloat(rawOrder.Price, 64)
	origQty, _ := strconv.ParseFloat(rawOrder.OrigQty, 64)
	executedQty, _ := strconv.ParseFloat(rawOrder.ExecutedQty, 64)
	cummulativeQuoteQty, _ := strconv.ParseFloat(rawOrder.CummulativeQuoteQty, 64)
	return &ProcessedOrderFull{
		Symbol:              rawOrder.Symbol,
		OrderID:             rawOrder.OrderID,
		OrderListID:         rawOrder.OrderListID,
		ClientOrderID:       rawOrder.ClientOrderID,
		TransactTime:        t,
		Price:               price,
		OrigQty:             origQty,
